...
func TestGuildDirect(t *testing.T) {
	t.Run("direct-list-guild", func(t *testing.T) {
		setup := guildDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-guild", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-guild", func(t *testing.T) {
		setup := guildDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-guild", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type guildDirectSetupResult struct {
...
func guildDirectSetup(mockres any) *guildDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"GUILD_WARS2_TEST_GUILD_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GUILD_WARS2_TEST_GUILD_ENTID"]; ok {
		...
		return &guildDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &guildDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
