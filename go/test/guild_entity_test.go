...
func TestGuildEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Guild(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Guild(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		guildRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.guild")))
...
func guildBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("GUILD_WARS2_TEST_GUILD_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GUILD_WARS2_TEST_GUILD_ENTID"])
