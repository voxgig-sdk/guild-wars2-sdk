...
func TestMiscellaneousEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Miscellaneous(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Miscellaneous(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		miscellaneousRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.miscellaneous")))
		...
		miscellaneousRef01DataDt0LoadResult := core.ToMapAny(entityData(miscellaneousRef01DataDt0Loaded))
...
func miscellaneousBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("GUILD_WARS2_TEST_MISCELLANEOUS_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GUILD_WARS2_TEST_MISCELLANEOUS_ENTID"])
