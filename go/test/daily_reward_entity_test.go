...
func TestDailyRewardEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.DailyReward(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.DailyReward(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		dailyRewardRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.daily_reward")))
...
func daily_rewardBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("GUILD_WARS2_TEST_DAILY_REWARD_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GUILD_WARS2_TEST_DAILY_REWARD_ENTID"])
