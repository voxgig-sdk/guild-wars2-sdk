...
func TestMapInformationEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		mapInformationRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.map_information")))
...
func map_informationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("GUILD_WARS2_TEST_MAP_INFORMATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GUILD_WARS2_TEST_MAP_INFORMATION_ENTID"])
