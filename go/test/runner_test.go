...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("GUILD_WARS2_TEST_LIVE") == "TRUE" ||
		os.Getenv("GUILD_WARS2_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("GUILD_WARS2_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.GuildWars2SDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.GuildWars2Error{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.GuildWars2SDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.GuildWars2Error{Msg: msg, Code: code}
}
