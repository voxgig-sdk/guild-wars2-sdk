...
func TestGuildAuthenticatedEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.GuildAuthenticated(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.GuildAuthenticated(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		guildAuthenticatedRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.guild_authenticated")))
...
func guild_authenticatedBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("GUILD_WARS2_TEST_GUILD_AUTHENTICATED_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GUILD_WARS2_TEST_GUILD_AUTHENTICATED_ENTID"])
