...
func TestHomeInstanceEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.HomeInstance(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.HomeInstance(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		homeInstanceRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.home_instance")))
...
func home_instanceBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("GUILD_WARS2_TEST_HOME_INSTANCE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GUILD_WARS2_TEST_HOME_INSTANCE_ENTID"])
