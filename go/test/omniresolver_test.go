...
import (
	...
	omni "github.com/voxgig-sdk/guild-wars2-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.GuildWars2Error); is {
				out := map[string]any{
					"name":    "GuildWars2Error",
...
func omniCtx(arg any, client *sdk.GuildWars2SDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.GuildWars2SDK); is {
			client = live
