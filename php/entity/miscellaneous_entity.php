...
// GuildWars2 SDK Miscellaneous entity
...
class MiscellaneousEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new MiscellaneousEntity($this->_client, $opts);
    }

    /**
     * @param Miscellaneous|array $args Miscellaneous data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = GuildWars2Helpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Miscellaneous|array The current Miscellaneous data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Miscellaneous fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = GuildWars2Helpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Miscellaneous fields).
     */
    ...
    /**
     * Load a single Miscellaneous.
     *
     * @param MiscellaneousLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed MiscellaneousLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Miscellaneous|array The loaded Miscellaneous as an assoc-array at the
     *   SDK boundary; throws GuildWars2Error on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = GuildWars2Helpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List Miscellaneous items matching the given filter.
     *
     * @param MiscellaneousListMatch|array|null $reqmatch Match filter (any subset
     *   of Miscellaneous fields) as an assoc-array; MiscellaneousListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Miscellaneous[]|array A list of Miscellaneous items as assoc-arrays at
     *   the SDK boundary; throws GuildWars2Error on failure (item-5 convention).
     */
