...
class GuildWars2RetryFeature extends GuildWars2BaseFeature
{
    ...
    public function init(GuildWars2Context $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (GuildWars2Context $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(GuildWars2Context $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(GuildWars2Context $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
