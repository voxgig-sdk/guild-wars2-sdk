...
class TradingPostEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = GuildWars2SDK::test(null, null);
        $ent = $testsdk->TradingPost(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->TradingPost(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->TradingPost(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = trading_post_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "trading_post." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GUILD_WARS2_TEST_TRADING_POST_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $trading_post_ref01_data = null;
        if (count($trading_post_ref01_data_raw) > 0) {
...
function trading_post_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/trading_post/TradingPostTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["trading_post01", "trading_post02", "trading_post03"] as $k) {
    ...
    $entid_env_raw = getenv("GUILD_WARS2_TEST_TRADING_POST_ENTID");
