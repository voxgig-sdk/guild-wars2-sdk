...
class GuildAuthenticatedDirectTest extends TestCase
{
    public function test_direct_list_guild_authenticated(): void
    {
        $setup = guild_authenticated_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-guild_authenticated", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["guild_authenticated01"] as $_liveKey) {
