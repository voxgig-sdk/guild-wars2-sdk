...
class WorldVsWorldEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = GuildWars2SDK::test(null, null);
        $ent = $testsdk->WorldVsWorld(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->WorldVsWorld(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->WorldVsWorld(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = world_vs_world_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "world_vs_world." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GUILD_WARS2_TEST_WORLD_VS_WORLD_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $world_vs_world_ref01_data = null;
        if (count($world_vs_world_ref01_data_raw) > 0) {
...
function world_vs_world_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/world_vs_world/WorldVsWorldTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["world_vs_world01", "world_vs_world02", "world_vs_world03"] as $k) {
    ...
    $entid_env_raw = getenv("GUILD_WARS2_TEST_WORLD_VS_WORLD_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["GUILD_WARS2_TEST_WORLD_VS_WORLD_ENTID"]);
