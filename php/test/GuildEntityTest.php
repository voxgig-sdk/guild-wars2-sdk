...
class GuildEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Guild(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Guild(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GUILD_WARS2_TEST_GUILD_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function guild_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("GUILD_WARS2_TEST_GUILD_ENTID");
