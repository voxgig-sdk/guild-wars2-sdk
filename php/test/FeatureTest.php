...
class FtCtrl extends GuildWars2Control
{
...
class FtClient
{
    ...
    private GuildWars2Utility $utility;

    public function __construct(GuildWars2Utility $utility, array $options)
    {
    ...
    public function get_utility(): GuildWars2Utility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (GuildWars2Context $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public GuildWars2Context $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (GuildWars2Context $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = GuildWars2Config::make_feature($name);
    ...
    private static function buildUrl(GuildWars2Spec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof GuildWars2Error) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new GuildWars2Response($fetched);
        }
    ...
    private function fail(GuildWars2Context $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (GuildWars2Context $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
