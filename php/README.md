...
The PHP SDK for the GuildWars2 API. Provides an entity-oriented interface using PHP conventions.
...
```php
[$result, $err] = $client->Achievement(null)->load(["id" => "example_id"], null);
if ($err) { throw new \Exception($err); }
print_r($result);
```
...
[$result, $err] = $client->GuildWars2(null)->load(
    ["id" => "test01"], null
);
