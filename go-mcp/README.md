...
[MCP](https://modelcontextprotocol.io) server exposing the GuildWars2 SDK as
two agent tools — `guild-wars2_list` and `guild-wars2_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/guild-wars2-mcp)
make build
...
```jsonc
// guild-wars2_list: first page of records
{ "entity": "achievement" }
{ "entity": "achievement", "query": { } }

// guild-wars2_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/guild-wars2-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user guild-wars2 \
     -- "$PWD"/dist/*/guild-wars2-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `guild-wars2_list` and `guild-wars2_load` tools now appear
   in new sessions. Ask the agent to *"list achievement using guild-wars2"*
   and it calls `guild-wars2_list` with `{"entity":"achievement"}`.
...
```sh
export GUILD_WARS2_APIKEY=sk_live_xxx            # API key
export GUILD_WARS2_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `guild-wars2_list` tool
...
### Call the `guild-wars2_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `guild-wars2_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `guild-wars2_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `GUILD_WARS2_APIKEY` | API key sent with every request. |
| `GUILD_WARS2_BASE` | Optional override of the API base URL. |
