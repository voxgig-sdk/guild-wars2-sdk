...
from guildwars2_sdk.guildwars2_types import (
    TradingPost,
    TradingPostLoadMatch,
    TradingPostListMatch,
)
...
class TradingPostEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return TradingPostEntity(self._client, opts)
    ...
    def match_get(self) -> TradingPost:
    ...
    def load(self, reqmatch=None, ctrl=None) -> TradingPost:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.TradingPost().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[TradingPost]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.TradingPost().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
