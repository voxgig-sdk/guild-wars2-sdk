...
from guildwars2_sdk.guildwars2_types import (
    Authenticated,
    AuthenticatedLoadMatch,
    AuthenticatedListMatch,
)
...
class AuthenticatedEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return AuthenticatedEntity(self._client, opts)
    ...
    def data_get(self) -> Authenticated:
    ...
    def match_get(self) -> Authenticated:
    ...
    def load(self, reqmatch=None, ctrl=None) -> Authenticated:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Authenticated().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Authenticated]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Authenticated().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
