...
class DailyRewardEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DailyRewardEntity(self._client, opts)
    ...
    def match_get(self) -> DailyReward:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[DailyReward]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.DailyReward().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
