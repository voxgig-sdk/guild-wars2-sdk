...
class TestWorldVsWorldDirect:

    def test_should_direct_list_world_vs_world(self):
        setup = _world_vs_world_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-world_vs_world", "live" if setup["live"] else "unit")
        if _skip:
...
def _world_vs_world_direct_setup(mockres):
