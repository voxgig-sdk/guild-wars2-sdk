...
class TestWorldVsWorldEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.WorldVsWorld(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.WorldVsWorld(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GUILD_WARS2_TEST_WORLD_VS_WORLD_ENTID JSON to run live")
        client = setup["client"]
...
def _world_vs_world_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("GUILD_WARS2_TEST_WORLD_VS_WORLD_ENTID"))
