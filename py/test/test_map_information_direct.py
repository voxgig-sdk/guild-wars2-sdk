...
class TestMapInformationDirect:

    def test_should_direct_list_map_information(self):
        setup = _map_information_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-map_information", "live" if setup["live"] else "unit")
        if _skip:
...
def _map_information_direct_setup(mockres):
