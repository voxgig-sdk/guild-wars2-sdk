...
class TestDailyRewardDirect:

    def test_should_direct_list_daily_reward(self):
        setup = _daily_reward_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-daily_reward", "live" if setup["live"] else "unit")
        if _skip:
...
def _daily_reward_direct_setup(mockres):
