...
class TestGameMechanicDirect:

    def test_should_direct_list_game_mechanic(self):
        setup = _game_mechanic_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-game_mechanic", "live" if setup["live"] else "unit")
        if _skip:
...
def _game_mechanic_direct_setup(mockres):
