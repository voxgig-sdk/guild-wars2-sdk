...
class TestStructuredPvPDirect:

    def test_should_direct_list_structured_pv_p(self):
        setup = _structured_pv_p_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-structured_pv_p", "live" if setup["live"] else "unit")
        if _skip:
...
def _structured_pv_p_direct_setup(mockres):
