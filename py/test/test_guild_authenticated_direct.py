...
class TestGuildAuthenticatedDirect:

    def test_should_direct_list_guild_authenticated(self):
        setup = _guild_authenticated_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-guild_authenticated", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["guild_authenticated01"]:
...
def _guild_authenticated_direct_setup(mockres):
