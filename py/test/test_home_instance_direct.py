...
class TestHomeInstanceDirect:

    def test_should_direct_list_home_instance(self):
        setup = _home_instance_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-home_instance", "live" if setup["live"] else "unit")
        if _skip:
...
def _home_instance_direct_setup(mockres):
