...
class GuildWars2TestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            GuildWars2TestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = GuildWars2TestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = GuildWars2TestRunner.getenv("GUILD_WARS2_TEST_LIVE")
        override = GuildWars2TestRunner.getenv("GUILD_WARS2_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = GuildWars2TestRunner.getenv(key)
        ...
        explain = GuildWars2TestRunner.getenv("GUILD_WARS2_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["GUILD_WARS2_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if GuildWars2TestRunner._test_control is not None:
            return GuildWars2TestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                GuildWars2TestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            GuildWars2TestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = GuildWars2TestRunner.load_test_control()
...
def load_env_local():
    GuildWars2TestRunner.load_env_local()


def env_override(m):
    return GuildWars2TestRunner.env_override(m)


def entity_data(v):
    return GuildWars2TestRunner.entity_data(v)


def entity_list_to_data(lst):
    return GuildWars2TestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return GuildWars2TestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return GuildWars2TestRunner.load_test_control()


def live_client_options():
    return GuildWars2TestRunner.live_client_options()


def live_delay_ms():
    return GuildWars2TestRunner.live_delay_ms()
