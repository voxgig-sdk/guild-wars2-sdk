...
boru-driven command-line client **and** interactive REPL for the GuildWars2
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/guild-wars2-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/guild-wars2-cli list achievement
   ```
...
```sh
./guild-wars2-cli load 1 achievement          # scalar shorthand for {id:1}
./guild-wars2-cli load '{id:1}' achievement     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `guild-wars2>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/guild-wars2-cli`. |
...
(`./guild-wars2-cli <expr>`) and interactively (the REPL), and expressions compose the
