// Command GuildWars2-cli is a CLI + REPL for the GuildWars2 SDK,
...
const prompt = "guild-wars2"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "achievement authenticated daily_reward game_mechanic guild guild_authenticated home_instance item map map_information miscellaneous story structured_pv_p trading_post world_vs_world"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      GUILD_WARS2_APIKEY (api key), GUILD_WARS2_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: GUILD_WARS2_APIKEY carries the API key and
	// GUILD_WARS2_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("GUILD_WARS2_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("GUILD_WARS2_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewGuildWars2SDK(opts)
