...
describe("HomeInstanceEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:HomeInstance(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:HomeInstance(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set GUILD_WARS2_TEST_HOME_INSTANCE_ENTID JSON to run live")
      return
    end
...
function home_instance_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("GUILD_WARS2_TEST_HOME_INSTANCE_ENTID")
