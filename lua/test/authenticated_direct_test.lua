...
describe("AuthenticatedDirect", function()
  it("should direct-list-authenticated", function()
    local setup = authenticated_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-authenticated", setup.live and "live" or "unit")
  ...
  it("should direct-load-authenticated", function()
    local setup = authenticated_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-authenticated", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function authenticated_direct_setup(mockres)
