...
describe("TradingPostDirect", function()
  it("should direct-list-trading_post", function()
    local setup = trading_post_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-trading_post", setup.live and "live" or "unit")
  ...
  it("should direct-load-trading_post", function()
    local setup = trading_post_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-trading_post", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function trading_post_direct_setup(mockres)
