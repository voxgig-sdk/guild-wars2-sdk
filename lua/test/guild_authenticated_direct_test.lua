-- GuildAuthenticated direct test
...
describe("GuildAuthenticatedDirect", function()
  it("should direct-list-guild_authenticated", function()
    local setup = guild_authenticated_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-guild_authenticated", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"guild_authenticated01"}) do
...
function guild_authenticated_direct_setup(mockres)
