...
describe("GuildDirect", function()
  it("should direct-list-guild", function()
    local setup = guild_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-guild", setup.live and "live" or "unit")
  ...
  it("should direct-load-guild", function()
    local setup = guild_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-guild", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function guild_direct_setup(mockres)
