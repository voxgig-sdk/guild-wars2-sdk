...
The Lua SDK for the GuildWars2 API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Achievement(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:GuildWars2(nil):load(
  { id = "test01" }, nil
)
