...
local HomeInstanceEntity = {}
HomeInstanceEntity.__index = HomeInstanceEntity


function HomeInstanceEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, HomeInstanceEntity)
  self._name = "home_instance"
  self._client = client
...
function HomeInstanceEntity:get_name()
  return self._name
end


function HomeInstanceEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return HomeInstanceEntity.new(self._client, opts)
end
...
function HomeInstanceEntity:mark_deleted()
  self._deleted = true
end


function HomeInstanceEntity:deleted()
  return true == self._deleted
end


function HomeInstanceEntity:data_set(args)
  if args ~= nil then
...
function HomeInstanceEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function HomeInstanceEntity:match_set(args)
  if args ~= nil then
...
function HomeInstanceEntity:match_get()
...
function HomeInstanceEntity:stream(action, args, callopts)
...
---@param reqmatch HomeInstanceListMatch
---@param ctrl? table
---@return HomeInstance[]
---@return string? err
function HomeInstanceEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function HomeInstanceEntity:_run_op(ctx, post_done)
  local utility = self._utility
