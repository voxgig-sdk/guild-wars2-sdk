...
local DailyRewardEntity = {}
DailyRewardEntity.__index = DailyRewardEntity


function DailyRewardEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DailyRewardEntity)
  self._name = "daily_reward"
  self._client = client
...
function DailyRewardEntity:get_name()
  return self._name
end


function DailyRewardEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DailyRewardEntity.new(self._client, opts)
end
...
function DailyRewardEntity:mark_deleted()
  self._deleted = true
end


function DailyRewardEntity:deleted()
  return true == self._deleted
end


function DailyRewardEntity:data_set(args)
  if args ~= nil then
...
function DailyRewardEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DailyRewardEntity:match_set(args)
  if args ~= nil then
...
function DailyRewardEntity:match_get()
...
function DailyRewardEntity:stream(action, args, callopts)
...
function DailyRewardEntity:_run_op(ctx, post_done)
  local utility = self._utility
