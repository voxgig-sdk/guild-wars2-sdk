...
local GameMechanicEntity = {}
GameMechanicEntity.__index = GameMechanicEntity


function GameMechanicEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GameMechanicEntity)
  self._name = "game_mechanic"
  self._client = client
...
function GameMechanicEntity:get_name()
  return self._name
end


function GameMechanicEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GameMechanicEntity.new(self._client, opts)
end
...
function GameMechanicEntity:mark_deleted()
  self._deleted = true
end


function GameMechanicEntity:deleted()
  return true == self._deleted
end


function GameMechanicEntity:data_set(args)
  if args ~= nil then
...
function GameMechanicEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GameMechanicEntity:match_set(args)
  if args ~= nil then
...
function GameMechanicEntity:match_get()
...
function GameMechanicEntity:stream(action, args, callopts)
...
---@param reqmatch GameMechanicListMatch
---@param ctrl? table
---@return GameMechanic[]
---@return string? err
function GameMechanicEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function GameMechanicEntity:_run_op(ctx, post_done)
  local utility = self._utility
