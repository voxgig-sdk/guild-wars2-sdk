...
local TradingPostEntity = {}
TradingPostEntity.__index = TradingPostEntity


function TradingPostEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TradingPostEntity)
  self._name = "trading_post"
  self._client = client
...
function TradingPostEntity:get_name()
  return self._name
end


function TradingPostEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TradingPostEntity.new(self._client, opts)
end
...
function TradingPostEntity:mark_deleted()
  self._deleted = true
end


function TradingPostEntity:deleted()
  return true == self._deleted
end


function TradingPostEntity:data_set(args)
  if args ~= nil then
...
function TradingPostEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TradingPostEntity:match_set(args)
  if args ~= nil then
...
function TradingPostEntity:match_get()
...
function TradingPostEntity:stream(action, args, callopts)
...
---@param reqmatch TradingPostLoadMatch
---@param ctrl? table
---@return TradingPost
---@return string? err
function TradingPostEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch TradingPostListMatch
---@param ctrl? table
---@return TradingPost[]
---@return string? err
function TradingPostEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function TradingPostEntity:_run_op(ctx, post_done)
  local utility = self._utility
