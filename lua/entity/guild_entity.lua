...
local GuildEntity = {}
GuildEntity.__index = GuildEntity


function GuildEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GuildEntity)
  self._name = "guild"
  self._client = client
...
function GuildEntity:get_name()
  return self._name
end


function GuildEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GuildEntity.new(self._client, opts)
end
...
function GuildEntity:mark_deleted()
  self._deleted = true
end


function GuildEntity:deleted()
  return true == self._deleted
end


function GuildEntity:data_set(args)
  if args ~= nil then
...
function GuildEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GuildEntity:match_set(args)
  if args ~= nil then
...
function GuildEntity:match_get()
...
function GuildEntity:stream(action, args, callopts)
...
---@param reqmatch GuildLoadMatch
---@param ctrl? table
---@return Guild
---@return string? err
function GuildEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch GuildListMatch
---@param ctrl? table
---@return Guild[]
---@return string? err
function GuildEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function GuildEntity:_run_op(ctx, post_done)
  local utility = self._utility
