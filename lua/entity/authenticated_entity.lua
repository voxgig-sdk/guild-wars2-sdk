...
local AuthenticatedEntity = {}
AuthenticatedEntity.__index = AuthenticatedEntity


function AuthenticatedEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AuthenticatedEntity)
...
function AuthenticatedEntity:get_name()
  return self._name
end


function AuthenticatedEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AuthenticatedEntity.new(self._client, opts)
end
...
function AuthenticatedEntity:mark_deleted()
  self._deleted = true
end


function AuthenticatedEntity:deleted()
  return true == self._deleted
end


function AuthenticatedEntity:data_set(args)
  if args ~= nil then
...
function AuthenticatedEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AuthenticatedEntity:match_set(args)
  if args ~= nil then
...
function AuthenticatedEntity:match_get()
...
function AuthenticatedEntity:stream(action, args, callopts)
...
function AuthenticatedEntity:_run_op(ctx, post_done)
  local utility = self._utility
