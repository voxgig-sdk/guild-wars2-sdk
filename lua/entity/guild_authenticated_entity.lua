-- GuildWars2 SDK GuildAuthenticated entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local GuildAuthenticatedEntity = {}
GuildAuthenticatedEntity.__index = GuildAuthenticatedEntity


function GuildAuthenticatedEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GuildAuthenticatedEntity)
  self._name = "guild_authenticated"
  self._client = client
...
function GuildAuthenticatedEntity:get_name()
  return self._name
end


function GuildAuthenticatedEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GuildAuthenticatedEntity.new(self._client, opts)
end
...
function GuildAuthenticatedEntity:mark_deleted()
  self._deleted = true
end


function GuildAuthenticatedEntity:deleted()
  return true == self._deleted
end


function GuildAuthenticatedEntity:data_set(args)
  if args ~= nil then
...
function GuildAuthenticatedEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GuildAuthenticatedEntity:match_set(args)
  if args ~= nil then
...
function GuildAuthenticatedEntity:match_get()
...
function GuildAuthenticatedEntity:stream(action, args, callopts)
...
---@param reqmatch GuildAuthenticatedListMatch
---@param ctrl? table
---@return GuildAuthenticated[]
---@return string? err
function GuildAuthenticatedEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function GuildAuthenticatedEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return GuildAuthenticatedEntity
