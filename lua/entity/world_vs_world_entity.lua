...
local WorldVsWorldEntity = {}
WorldVsWorldEntity.__index = WorldVsWorldEntity


function WorldVsWorldEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WorldVsWorldEntity)
  self._name = "world_vs_world"
  self._client = client
...
function WorldVsWorldEntity:get_name()
  return self._name
end


function WorldVsWorldEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WorldVsWorldEntity.new(self._client, opts)
end
...
function WorldVsWorldEntity:mark_deleted()
  self._deleted = true
end


function WorldVsWorldEntity:deleted()
  return true == self._deleted
end


function WorldVsWorldEntity:data_set(args)
  if args ~= nil then
...
function WorldVsWorldEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WorldVsWorldEntity:match_set(args)
  if args ~= nil then
...
function WorldVsWorldEntity:match_get()
...
function WorldVsWorldEntity:stream(action, args, callopts)
...
function WorldVsWorldEntity:_run_op(ctx, post_done)
  local utility = self._utility
