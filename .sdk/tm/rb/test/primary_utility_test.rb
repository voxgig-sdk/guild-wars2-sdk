...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = GuildWars2Omni.make_runner(TEST_JSON_FILE, GuildWars2SDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    GuildWars2Error.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : GuildWars2Omni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = GuildWars2Operation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
