...
class TradingPostEntity
  ...
  def make
    opts = @_entopts.dup
    TradingPostEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.TradingPost.load works with no args).
  ...
  #
  # @param reqmatch [TradingPostListMatch, Hash, nil] match filter (any subset of
  #   TradingPost fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
