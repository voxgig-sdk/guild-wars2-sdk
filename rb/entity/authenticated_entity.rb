...
class AuthenticatedEntity
  ...
  def make
    opts = @_entopts.dup
    AuthenticatedEntity.new(@_client, opts)
  end
  ...
  # @return [Authenticated, Hash] the current Authenticated data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Authenticated fields)
  def match_get
  ...
  #
  # @param reqmatch [AuthenticatedLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Authenticated.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Authenticated, Hash] the loaded Authenticated; raises GuildWars2Error on failure
  ...
  #
  # @param reqmatch [AuthenticatedListMatch, Hash, nil] match filter (any subset of
  #   Authenticated fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Authenticated>, Array] the matching Authenticated items; raises GuildWars2Error on failure
