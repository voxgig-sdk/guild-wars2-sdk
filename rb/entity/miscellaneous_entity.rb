...
class MiscellaneousEntity
  ...
  def make
    opts = @_entopts.dup
    MiscellaneousEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = GuildWars2Helpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Miscellaneous, Hash] the current Miscellaneous data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = GuildWars2Helpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Miscellaneous fields)
  def match_get
  ...
  #
  # @param reqmatch [MiscellaneousLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Miscellaneous.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Miscellaneous, Hash] the loaded Miscellaneous; raises GuildWars2Error on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = GuildWars2Helpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [MiscellaneousListMatch, Hash, nil] match filter (any subset of
  #   Miscellaneous fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Miscellaneous>, Array] the matching Miscellaneous items; raises GuildWars2Error on failure
