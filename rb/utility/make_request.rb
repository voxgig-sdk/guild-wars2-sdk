...
module GuildWars2Utilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = GuildWars2Response.new({})
    ...
    if fetch_err
      ...
      response = GuildWars2Response.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = GuildWars2Response.new(fetched)
    else
