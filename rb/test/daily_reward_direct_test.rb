...
class DailyRewardDirectTest < Minitest::Test
  def test_direct_list_daily_reward
    setup = daily_reward_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-daily_reward", setup[:live] ? "live" : "unit")
    if _should_skip
...
def daily_reward_direct_setup(mockres)
