...
class GameMechanicDirectTest < Minitest::Test
  def test_direct_list_game_mechanic
    setup = game_mechanic_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-game_mechanic", setup[:live] ? "live" : "unit")
    if _should_skip
...
def game_mechanic_direct_setup(mockres)
