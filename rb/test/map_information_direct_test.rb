...
class MapInformationDirectTest < Minitest::Test
  def test_direct_list_map_information
    setup = map_information_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-map_information", setup[:live] ? "live" : "unit")
    if _should_skip
...
def map_information_direct_setup(mockres)
