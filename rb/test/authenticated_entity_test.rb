...
class AuthenticatedEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Authenticated(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Authenticated(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GUILD_WARS2_TEST_AUTHENTICATED_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    authenticated_ref01_data_dt0_load_result = Helpers.to_map(authenticated_ref01_data_dt0_loaded.respond_to?(:data_get) ? authenticated_ref01_data_dt0_loaded.data_get : authenticated_ref01_data_dt0_loaded)
...
def authenticated_basic_setup(extra)
  ...
  entid_env_raw = ENV["GUILD_WARS2_TEST_AUTHENTICATED_ENTID"]
