...
class WorldVsWorldDirectTest < Minitest::Test
  def test_direct_list_world_vs_world
    setup = world_vs_world_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-world_vs_world", setup[:live] ? "live" : "unit")
    if _should_skip
...
def world_vs_world_direct_setup(mockres)
