...
class WorldVsWorldEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.WorldVsWorld(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.WorldVsWorld(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = world_vs_world_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "world_vs_world." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GUILD_WARS2_TEST_WORLD_VS_WORLD_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    world_vs_world_ref01_data = nil
    if world_vs_world_ref01_data_raw.length > 0
...
def world_vs_world_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "world_vs_world", "WorldVsWorldTestData.json")
  ...
  entid_env_raw = ENV["GUILD_WARS2_TEST_WORLD_VS_WORLD_ENTID"]
