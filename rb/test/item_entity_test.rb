...
class ItemEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GUILD_WARS2_TEST_ITEM_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def item_basic_setup(extra)
  ...
  entid_env_raw = ENV["GUILD_WARS2_TEST_ITEM_ENTID"]
  ...
  idmap_resolved = Helpers.to_map(
    env["GUILD_WARS2_TEST_ITEM_ENTID"])
