...
class GuildAuthenticatedDirectTest < Minitest::Test
  def test_direct_list_guild_authenticated
    setup = guild_authenticated_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-guild_authenticated", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["guild_authenticated01"].each do |_live_key|
...
def guild_authenticated_direct_setup(mockres)
