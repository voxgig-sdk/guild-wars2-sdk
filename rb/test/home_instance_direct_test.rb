...
class HomeInstanceDirectTest < Minitest::Test
  def test_direct_list_home_instance
    setup = home_instance_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-home_instance", setup[:live] ? "live" : "unit")
    if _should_skip
...
def home_instance_direct_setup(mockres)
