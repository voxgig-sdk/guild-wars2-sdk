...
class GuildDirectTest < Minitest::Test
  def test_direct_list_guild
    setup = guild_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-guild", setup[:live] ? "live" : "unit")
    if _should_skip
  ...
  def test_direct_load_guild
    setup = guild_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-guild", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
...
def guild_direct_setup(mockres)
