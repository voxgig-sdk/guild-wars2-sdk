...
describe('TradingPostDirect', async () => {
  ...
  test('direct-load-trading_post', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-trading_post', setup.live)) return
  ...
  test('direct-list-trading_post', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-trading_post', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GUILD_WARS2_TEST_TRADING_POST_ENTID']
