...
describe('GuildDirect', async () => {
  ...
  test('direct-load-guild', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-guild', setup.live)) return
  ...
  test('direct-list-guild', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-guild', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GUILD_WARS2_TEST_GUILD_ENTID']
