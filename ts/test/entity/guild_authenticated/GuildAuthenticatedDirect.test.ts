...
describe('GuildAuthenticatedDirect', async () => {
  ...
  test('direct-list-guild_authenticated', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-guild_authenticated', setup.live)) return
    if (skipIfMissingIds(t, setup, ["guild_authenticated01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GUILD_WARS2_TEST_GUILD_AUTHENTICATED_ENTID']
