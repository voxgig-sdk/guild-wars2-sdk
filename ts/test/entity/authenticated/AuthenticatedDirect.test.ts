...
describe('AuthenticatedDirect', async () => {
  ...
  test('direct-load-authenticated', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-authenticated', setup.live)) return
  ...
  test('direct-list-authenticated', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-authenticated', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GUILD_WARS2_TEST_AUTHENTICATED_ENTID']
